...
if (isset($_POST['edit_post'])) {
  if (trim($_POST['title'])!="" && trim($_POST['author'])!="" && trim($_POST['category_id'])!="" && trim($_POST['body'])!="") {
    ...
    if (trim($_FILES['image']['name'])!="") {
      ...
      $post_insert = $db->prepare("UPDATE posts SET title=:title, author=:author, category_id=:category_id, body=:body, image=:image WHERE id=:id");
      $post_insert->execute(['title'=>$title, 'author'=>$author, 'category_id'=>$category_id, 'body'=>$body, 'image'=>$name_image, 'id'=>$post_id]);

    }else {
      $post_insert = $db->prepare("UPDATE posts SET title=:title, author=:author, category_id=:category_id, body=:body WHERE id=:id");
      $post_insert->execute(['title'=>$title, 'author'=>$author, 'category_id'=>$category_id, 'body'=>$body, 'id'=>$post_id]);

    }
  }else {
    header("Location:edit_post.php?id=$post_id&err_msg=تمام فیلدها الزامی است");
    exit();
  }
}
   ...
   <div class="row">
     ...
     <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
       ...
       <form class="mb-5" method="post" enctype="multipart/form-data">
         ...
         <img src="../upload/posts/<?php echo $post['image'] ?>" alt="">
         <div class="form-group">
